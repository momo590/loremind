...
def _run_processor(mock_store, claude_tool_response, entities_payload, session_number=1, raw_text="notes"):
    with patch("loremind.llm.claude_provider.anthropic.Anthropic") as mock_cls:
        mock_client = MagicMock()
        mock_cls.return_value = mock_client
        mock_client.messages.create.return_value = claude_tool_response(entities_payload)
...
def test_brask_and_captain_brask_merge(mock_store, claude_tool_response):
    ...
    _run_processor(
        mock_store, claude_tool_response,
        [{
            "name": "Brask",
            "entity_type": "npc",
...
def test_below_threshold_creates_new(mock_store, claude_tool_response):
    """'Brask' vs 'Brock' should score below 85 on partial_ratio
...
def test_processor_passes_existing_names_to_llm(mock_store, claude_tool_response):
    ...
    assert "Brask" in captured["prompt"]
    assert "Known canonical entity names" in captured["prompt"]
